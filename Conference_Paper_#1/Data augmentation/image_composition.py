...
class ImageComposition():
    ...
    def _validate_and_process_args(self, args):
        ...
        # validate the count
        assert args.count > 0, 'count must be greater than 0' # if the result is fail, the exception will be appeared
    ...
    def _generate_images(self, i_0):  # them this code
        # Generate a number of images and creates segmentation masks,
        # then saves a mask_definitions.json file that describle the dataset
        ...
        for i in tqdm(range(i_0, self.count + i_0)):    # them this code
            ...
            # Create the file name(use for both composite and mask)
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        for fg in foregrounds:
            ...
            max_y_position = composite.size[0] - fg_image.size[1]
            assert max_x_position >= 0 and max_y_position >= 0,f'foreground {fg_path} is too big ({fg_image.size[0]}x{fg_image.size[1]}) for the requested output size ({self.width}x{self.height}), check your input parameters'
            paste_position = (random.randint(0, max_x_position), random.randint(0, max_y_position))

            # Create a new foreground images as large as the composite and paste it on top
    ...
    def main(self, args):
        ...
        self._generate_images(args.num_dataset_1) # them this code
        self._create_info()
        print('Image compositon completed')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_dataset_1", type=int, dest="num_dataset_1", default=0, help="number of previous created images") # them this code
